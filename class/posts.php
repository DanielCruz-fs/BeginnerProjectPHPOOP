<?php
   ...
   class Posts extends Conexion{
      ...
      //-----------------------------------------------------------------------------------------------------------
      public function all(){
         $resultado = $this->mysqli->query("SELECT * FROM posts INNER JOIN categorias ON categorias.idcategoria=posts.idcategoria ORDER BY posts.idpost DESC");
      ...
      public function delete($id){
         $resultado = $this->mysqli->query("DELETE FROM posts WHERE idpost=$id");
         $resultado_cascada = $this->mysqli->query("DELETE FROM comentarios WHERE idpost=$id");
         header("Location: index_post.php");
      }
      //-----------------------------------------------------------------------------------------------------------
		public function find($id){
			if( !($sentencia = $this->mysqli->prepare("SELECT * FROM posts WHERE idpost = ?") ) ){
      ...
      //-----------------------------------------------------------------------------------------------------------
		public function insert(){
			...
			if (!($sentencia = $this->mysqli->prepare("INSERT INTO posts(idusuario,idcategoria,titulo,subtitulo,cuerpo,fecha) VALUES (?,?,?,?,?,?)")))
				 echo "Falló la preparación: (" . $this->mysqli->errno . ") " . $this->mysqli->error;
			
			/* Sentencia preparada, etapa 2: vinculación y ejecución */
			if (!$sentencia->bind_param('iissss', $_POST['idusuario'], $_POST['idcategoria'], $_POST['titulo'], $_POST['subtitulo'], $_POST['cuerpo'], $_POST['fecha']))
